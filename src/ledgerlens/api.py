    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        while True:
            ...
            if len(content)>MAX_BYTES:
                await JSONResponse({'detail':'Request exceeds 10 MB'},status_code=413)(scope,receive,send)
                return
            if not message.get('more_body',False):
                break
        replayed = False
        async def replay() -> dict:  # type: ignore[type-arg]
            nonlocal replayed
            if replayed:
...
async def import_facts(request: Request) -> JsonObject:
    try:
        ...
        raise HTTPException(status_code=422,detail='Invalid SEC companyfacts JSON: '+str(exc)[:400]) from exc
