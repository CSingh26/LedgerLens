...
def analyze_connections(periods: list[JsonObject], query: Query) -> JsonObject:
    ...
    opening_aligned = (
        prior is not None
        and 1 <= (query.period_start - date.fromisoformat(prior["end"])).days <= 14
    )
    ...
    def invested(period: JsonObject) -> float | None:
        return difference(
            total(
                *(
                    value(period, k)
                    for k in ["short_term_borrowings", "current_debt", "long_term_debt", "equity"]
                )
            ),
            value(period, "cash"),
        )
    ...
    average = (
        (current_capital + prior_capital) / 2
        if current_capital is not None and prior_capital is not None
        else None
    )
    ...
    return {
        "cash_bridge": {
            "opening": opening("cash_flow_balance"),
            "cfo": value(current, "cfo"),
            "cfi": value(current, "cfi"),
            "cff": value(current, "cff"),
            "fx": value(current, "fx_cash"),
            "net_flows": cash_flows,
            "closing": closing_cash,
            "residual": difference(cash_change, cash_flows),
        },
        "income_to_cfo_adjustments": difference(
            value(current, "cfo"), value(current, "net_income")
        ),
        "retained_earnings_change": difference(
            value(current, "retained_earnings"), opening("retained_earnings")
        ),
        "retained_earnings_other_changes": difference(
            difference(value(current, "retained_earnings"), opening("retained_earnings")),
        ...
        "notes": [
            "Cash bridge requires the same cash+restricted-cash scope and every cash-flow component; no missing FX value becomes zero.",
            ...
            "ROIC requires explicit short-term borrowings and uses debt + equity − all cash and normalized taxes. Review excess cash, leases, minority interests and tax-loss carryforwards.",
        ],
    }
