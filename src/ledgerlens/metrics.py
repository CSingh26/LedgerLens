...
def statement_metrics(period: JsonObject) -> dict[str, float | None]:
    ...
    debt = total(get('current_debt'), get('long_term_debt'))
    return {
        ...
        'debt': debt, 'net_debt': difference(debt, get('cash')),
        'debt_to_equity': ratio(debt, get('equity')),
        'interest_coverage': ratio(get('ebit'), get('interest')),
        'balance_sheet_residual': difference(difference(get('assets'), get('liabilities')), get('equity')),
    }
