...
def parse_companyfacts(content: bytes, source: str = 'USER PROVIDED DATA', url: str | None = None) -> JsonObject:
    if len(content) > MAX_BYTES:
        raise ValueError('Companyfacts exceeds 10 MB')
    facts = CompanyFacts.model_validate_json(content)
    return {'companyfacts': facts.model_dump(mode='json'), 'metadata': {
        'source': source, 'source_url': url, 'retrieved_at': datetime.now(timezone.utc).isoformat(),
...
def fetch_companyfacts(cik: int, user_agent: str, client: httpx.Client | None = None) -> JsonObject:
    ...
    if not 8 <= len(user_agent) <= 200 or not re.search(r'\S+@\S+\.\S+', user_agent) or '\n' in user_agent or '\r' in user_agent:
    ...
    try:
        with client.stream('GET', url, headers={'User-Agent': user_agent, 'Accept': 'application/json'}) as response:
            if response.status_code != 200:
                raise ProviderError(f'SEC data unavailable (HTTP {response.status_code}); retry later or upload companyfacts JSON')
            content = bytearray()
        ...
        raise ProviderError('SEC data unavailable due to network/timeout; no replacement data supplied') from exc
