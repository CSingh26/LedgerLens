...
def derive_quarterly_flows(data: CompanyFacts, query: Query, period: JsonObject) -> JsonObject:
    ...
    for name in CUMULATIVE:
        ...
        current = [
            (tag, unit, row)
            for tag, unit, row in eligible
            if row.accn == period["accession"]
            and row.filed == cutoff
            and row.end == query.period_end
            and row.start is not None
            and row.start < query.period_start
            and (row.end - row.start).days <= 310
        ]
        if not current:
            continue
        ...
        prior = [
            (t, u, row)
            for t, u, row in eligible
            if t == tag
            and row.start == ending.start
            and row.end == query.period_start - timedelta(days=1)
            and row.filed <= cutoff
        ]
        if not prior:
            continue
        ...
        if len({r.val for r in matches}) != 1:
            ...
            continue
        result["facts"][name] = {
            **evidence(tag, unit, ending),
            "value": ending.val - latest.val,
            "start": str(query.period_start),
            "kind": "derived_ytd_difference",
            "sources": [evidence(tag, unit, ending), evidence(tag, unit, latest)],
        }
        result["warnings"].append(
            f"{name} is current YTD minus prior YTD using disclosed accessions; review classification/restatement consistency."
        )
    return result
