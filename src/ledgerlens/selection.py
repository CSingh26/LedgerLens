...
TAGS: dict[str, tuple[str, ...]] = {
 ...
 'gross_profit': ('GrossProfit',), 'ebit': ('OperatingIncomeLoss',),
 'net_income': ('NetIncomeLoss','ProfitLoss'), 'cfo': ('NetCashProvidedByUsedInOperatingActivities',),
 ...
 'current_liabilities': ('LiabilitiesCurrent',), 'receivables': ('AccountsReceivableNetCurrent',),
 'inventory': ('InventoryNet',), 'current_debt': ('LongTermDebtCurrent',),
 'long_term_debt': ('LongTermDebtNoncurrent',), 'interest': ('InterestExpense',),
 'eps': ('EarningsPerShareDiluted',), 'diluted_shares': ('WeightedAverageNumberOfDilutedSharesOutstanding',),
...
INSTANT = {'assets','liabilities','equity','cash','current_assets','current_liabilities',
           'receivables','inventory','current_debt','long_term_debt','retained_earnings','cash_flow_balance'}
...
def rows_for(data: CompanyFacts, name: str, query: Query) -> list[tuple[str, str, SecFact]]:
    ...
    for tag in TAGS[name]:
        ...
        if concept:
            output.extend((tag, unit, row) for row in concept.units.get(unit, [])
...
def evidence(tag: str, unit: str, row: SecFact) -> JsonObject:
    return {'value': row.val, 'tag': tag, 'unit': unit, 'start': str(row.start) if row.start else None,
            'end': str(row.end), 'filed': str(row.filed), 'accession': row.accn,
            'form': row.form, 'kind': 'structured_fact'}


def select_period(data: CompanyFacts, query: Query, *, accession: str | None = None,
                  filed: date | None = None) -> JsonObject:
    revenue = [r for _, _, r in rows_for(data, 'revenue', query)
               if r.start == query.period_start and r.end == query.period_end
               and (accession is None or r.accn == accession) and (filed is None or r.filed == filed)]
    if not revenue:
    ...
    selected: JsonObject = {}
    for name in TAGS:
        matches = [(tag, unit, row) for tag, unit, row in rows_for(data, name, query)
                   if row.accn == anchor.accn and row.filed == anchor.filed and row.end == query.period_end
    ...
    return {'facts': selected, 'accession': anchor.accn, 'filed': str(anchor.filed),
            'start': str(query.period_start), 'end': str(query.period_end), 'form': query.form,
