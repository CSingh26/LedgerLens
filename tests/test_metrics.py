...
def test_hand_calculated_statements_and_liquidity():
 ...
 assert m['gross_margin']==pytest.approx(.4)
 assert m['operating_margin']==pytest.approx(.2)
 assert m['net_margin']==pytest.approx(.12)
...
def test_missing_or_nonpositive_denominators_are_unavailable():
 p=select_period(facts(),query());p['facts']['current_liabilities']['value']=0
 p['facts']['depreciation']=None;p['facts']['capex']=None;p['facts']['net_income']['value']=-1
