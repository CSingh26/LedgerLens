...
def test_offline_import_records_hash_and_rejects_bad_json():
    ...
    with pytest.raises(ValueError): parse_companyfacts(b'{broken')
    with pytest.raises(ValueError): parse_companyfacts(b'x'*10000001)
...
def test_provider_errors_never_substitute_demo(status):
    with httpx.Client(transport=httpx.MockTransport(lambda request:httpx.Response(status))) as client:
        with pytest.raises(ProviderError): fetch_companyfacts(1,'Research user@company.org',client=client)


def test_missing_user_agent_rejected_before_network():
    with pytest.raises(ValueError): fetch_companyfacts(1,'')
