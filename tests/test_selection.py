...
def test_exact_accession_units_period_and_provenance():
    ...
    assert (
        result["facts"]["revenue"]["tag"] == "RevenueFromContractWithCustomerExcludingAssessedTax"
    )
...
def test_future_and_wrong_duration_not_blended():
    data = facts()
    q = query().model_copy(update={"as_of": query().period_end})
    with pytest.raises(ValueError, match="revenue"):
        select_period(data, q)
    data.facts["us-gaap"]["NetCashProvidedByUsedInOperatingActivities"].units["USD"][
        0
    ].start = query().period_end
    assert select_period(data, query())["facts"]["cfo"] is None


def test_conflicting_duplicates_and_different_accession_unavailable():
    data = facts()
    rows = data.facts["us-gaap"]["OperatingIncomeLoss"].units["USD"]
    ...
    data = facts()
    data.facts["us-gaap"]["GrossProfit"].units["USD"][0].accn = "0000000001-26-000099"
